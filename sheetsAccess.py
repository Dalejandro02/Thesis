...
try:
    esp_ser = serial.Serial(port='COM3', baudrate=115200, bytesize=serial.EIGHTBITS,
                            parity=serial.PARITY_NONE, stopbits=serial.STOPBITS_ONE, timeout=1)
    print("Conexión serial con ESP8266 establecida en COM3.")
except Exception as e:
    print(f"Error al configurar el puerto serial de la ESP8266: {e}")
    esp_ser = None

try:
    vb_ser = serial.Serial(port='COM1', baudrate=9600, bytesize=serial.EIGHTBITS,
                           parity=serial.PARITY_NONE, stopbits=serial.STOPBITS_ONE, timeout=1)
    print("Conexión serial con el videobeam establecida en COM1.")
except Exception as e:
    print(f"Error al configurar el puerto serial del videobeam: {e}")
    vb_ser = None
...
def send_videobeam_command(command):
    try:
        if vb_ser and vb_ser.is_open:
            vb_ser.write(command.encode())
            print(f"Comando enviado al videobeam: {command.strip()}")
        else:
            print("El puerto serial del videobeam no está abierto.")
    except Exception as e:
        print(f"Error al enviar el comando al videobeam: {e}")
